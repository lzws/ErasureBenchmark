...
def generate_images(evaluation_task, model_name, prompts_path, save_path, device='cuda:0', guidance_scale = 7.5, image_size=512, ddim_steps=100, num_samples=10, from_case=0):
    ...
    print(f'AdvUnlearn 任务：{evaluation_task},加载模型：{model_name}， 保存路径：{folder_path}')
...
if __name__=='__main__':
    ...
    parser.add_argument('--erase_concept', help='erase_concpet', type=str, required=True)
    ...
    parser.add_argument('--ddim_steps', help='ddim steps of inference used to train', type=int, required=False, default=40)
    ...
    generate_images(model_name, erase_concept, prompts_path, save_path, device=device,
                    guidance_scale = guidance_scale, image_size=image_size, ddim_steps=ddim_steps, num_samples=num_samples,from_case=from_case)


# /home/users/diffusion/project/EraseConceptBenchmark/image_result/nsfw/nudity/AdvUnlearn/i2p
# nohup ATPipeline.py --model_name 'SD-v1-4' --erase_concept 'nudity' --prompts_path '/home/users/diffusion/project/EraseConceptBenchmark/dataset/i2p_4703.csv'