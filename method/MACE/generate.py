...
def MACE_generate_images(evaluation_task, model_name, prompts_path, save_path, step, device='cuda:0', guidance_scale = 7.5, image_size=512, ddim_steps=100, num_samples=1, from_case=0):
    ...
    for i in range(0, len(df), step): 
        ...
        if evaluation_task == "i2p":
            filename = f"{row.case_number}.png"
        ...
        if os.path.isfile(os.path.join(folder_path, filename)):
            ...
            images = pipe(prompt, num_inference_steps=ddim_steps, guidance_scale=guidance_scale, num_images_per_prompt=num_samples, 
