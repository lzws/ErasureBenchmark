...
def SalUnGenerate(evaluation_task, model_name, prompts_path, save_path, device="cuda:1",
                  guidance_scale=7.5, image_size=512, ddim_steps=40,
                  num_samples=1, from_case=0):
    # 1. Load the autoencoder model which will be used to decode the latents into image space.
    vae = AutoencoderKL.from_pretrained(
        model_path['SD14'], subfolder="vae"
    )
    # 2. Load the tokenizer and text encoder to tokenize and encode the text.
    tokenizer = CLIPTokenizer.from_pretrained(
        model_path['SD14'], subfolder="tokenizer"
    )
    text_encoder = CLIPTextModel.from_pretrained(model_path['SD14'], subfolder="text_encoder")
    # 3. The UNet model for generating the latents.
    unet = UNet2DConditionModel.from_pretrained(
        model_path['SD14'], subfolder="unet"
    )

    unet.load_state_dict(torch.load(model_name))
    ...
    print(f'SaiUn 任务: {evaluation_task}，加载模型：{model_name}, 保存地址: {save_path}')
    ...
    for _, row in df.iterrows():
        ...
        pil_images = [Image.fromarray(image) for image in images]
        for num, im in enumerate(pil_images):
            im.save(f"{save_path}/{case_number}.png")
