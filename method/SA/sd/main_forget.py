...
if __name__ == "__main__":
    ...
    try:
        ...
        default_callbacks_cfg = {
            "setup_callback": {
                "target": "main_forget.SetupCallback",
                "params": {
                    "resume": opt.resume,
                    "now": now,
                    "logdir": logdir,
            ...
            "learning_rate_logger": {
                "target": "main_forget.LearningRateMonitor",
                "params": {
            ...
            "cuda_callback": {
                "target": "main_forget.CUDACallback"
            },
        }
        if version.parse(pl.__version__) >= version.parse('1.4.0'):
        ...
        if opt.train:
            try:
                ...
                # model = model.cuda(0)
                print("---"*20)
                # print("qqqqqqqq model.device: ",model.device)
                # model.to('cuda:0')
